...
class VineFeeder(QWidget):
    ...
    '''def create_service_buttons(self):
        """Create buttons for each dynamically loaded service."""
        for service_name in self.available_services.keys():
                    ...
                    if hasattr(loader_instance, 'receive'):
                        ...
                        if text_to_pass:
                            if 'http' in text_to_pass:  
                                loader_instance.receive(1, text_to_pass)  # inx 1 signifies direct download
                            else:
